...
```{r}
lm_v2 <- lm(ebl ~ race + sex + age +  sex + ven_res + neo_chemo + 
              neo_xrt + liver_test*les_size + age*or_time,
            data = pancreatic_df)
anova(lm_v2, lm_v1)
...
lm_v4 <- lm(ebl ~ race*sex + age +  sex + ven_res + neo_chemo + 
              neo_xrt + liver_test + les_size + age*or_time,
            data = pancreatic_df)
anova(lm_v4, lm_v1)
```
...
get_quintile_missing_pct <- lapply(1:length(colnames), function(x){
  ...
  plot <- ggplot(df_filter,
                 aes(x = x_percentile, y = pct_missing_y, color = sex)) +
...
gam_v1 <- gam(ebl ~ race*sex + age +  sex + ven_res + neo_chemo + 
              neo_xrt + s(liver_test) + s(les_size) + ti(liver_test, les_size) + age*or_time,
            data = pancreatic_df)
...
gam_v2 <- gam(ebl ~ race*sex + age +  sex  + ven_res + neo_chemo + 
              neo_xrt + s(liver_test) + s(log(les_size)) + ti(liver_test, log(les_size)) + age*or_time,
            data = pancreatic_df)
...
allmods_diag <- data.frame(
  Model = c("LM", "GAM_v1", "GAM_v2"),
  AIC   = c(AIC(lm_v1), AIC(gam_v1), AIC(gam_v2)),
  BIC   = c(BIC(lm_v1), BIC(gam_v1), BIC(gam_v2)),
  `log(Likelihood)` = c(as.numeric(logLik(lm_v1)), as.numeric(logLik(gam_v1)), 
             as.numeric(logLik(gam_v2))), 
  DF     = c(attr(logLik(lm_v1), "df"),
             attr(logLik(gam_v1), "df"), 
...
pdf("figures_tables/.pdf", width = 8, height = 8)
plot(gam_v1, shade = TRUE, pages = 1, 
     xlab = "liver_test", ylab = "s(liver_test), edf = 8.735")
plot(gam_v1, shade = TRUE, pages = 1, 
     xlab = "les_size", ylab = "s(les_size), edf = 7.134")
dev.off()
plot(gam_v1, shade = TRUE, pages = 1, 
     xlab = "liver_test", ylab = "s(liver_test), edf = 8.735")
plot(gam_v1, shade = TRUE, pages = 1, 
...
plot_model(gam_v1, type = "pred", terms = c("or_time", "sex", "race")) +
  labs(title = "", 
...
```{r, fig.align='center', fig.width=7, fig.height=4, warning = FALSE, message = FALSE}
plot_model(gam_v1, type = "pred", terms = c("liver_test", "les_size", "sex", "race")) +
  labs(title = "", 
       y = "Estimated Blood Loss (in mL) Prediction",
     x = "Bilirubin Level (in mg/dL)") +
  theme_classic()
ggsave("figures_tables/ebl_pred_race_gender_liver_test_tumorsize.pdf", height = 7, width =8)
```
Same trend in men and women across all 4 races

```{r, fig.align='center', fig.width=4, fig.height=4, warning = FALSE, message = FALSE}
plot_model(gam_v1, type = "pred", terms = c("sex", "race"), dodge = 0.6) +
  labs(title = "", 
       y = "Estimated Blood Loss (in mL) Prediction",
     x = "") +
  theme_classic() +
  coord_flip()
ggsave("figures_tables/ebl_pred_race_gender.pdf", height = 3, width =4)
```

```{r, fig.align='center', fig.width=5, fig.height=4, warning = FALSE, message = FALSE}
plot_model(gam_v1, type = "pred", terms = c("neo_chemo", "neo_xrt"), dodge = 0.6) +
  labs(title = "", 
       y = "Estimated Blood Loss (in mL) Prediction",
     x = "Prior Chemotherapy", color = "Prior Radiation Therapy") +
  theme_classic() +
  coord_flip()
ggsave("figures_tables/ebl_pred_chem_rad_therapy.pdf", height = 3, width =5)
```
